-- Allow supervisors to move all leads or only leads with a selected status.
drop function if exists public.reassign_sales_leads(uuid, uuid[]);

create function public.reassign_sales_leads(source_owner uuid, target_owners uuid[], source_status text)
returns integer
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  with ranked as (
    ...
    where owner_id = source_owner
      and (source_status is null or status = source_status)
  )
...
revoke all on function public.reassign_sales_leads(uuid, uuid[], text) from public;
grant execute on function public.reassign_sales_leads(uuid, uuid[], text) to authenticated;
